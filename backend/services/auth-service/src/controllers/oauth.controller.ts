...
export class OAuthController {
  ...
  async googleCallback(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!tokenData.access_token) return res.status(400).json({ error: 'Failed to fetch access token', details: tokenData });
      ...
      if (!profile.id || !profile.email) return res.status(400).json({ error: 'Failed to fetch profile' });
  ...
  githubLogin(req: Request, res: Response) {
    ...
    if (!clientId || !redirectUri) return res.status(500).json({ error: 'GitHub OAuth not configured' });
  ...
  async githubCallback(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!profile.id || !email) return res.status(400).json({ error: 'Failed to fetch profile or email' });
  ...
  private setCookiesAndRedirect(res: Response, accessToken: string, refreshToken: string) {
